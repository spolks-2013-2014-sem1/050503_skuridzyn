...
def __udp_client_routine(client, filename, host, port):
	...
	server = (host, port)
        f = open(filename, 'rb')
        f_size = filework.get_fsize(f)
	packet_number = int(ceil(float(f_size) / (__UDP_BUF_SIZE))) + 1
        bytes_sended = 0

        while True:
		seek = f.tell()
                buffer = str(packet_number).rjust(16, '0') + str(seek).rjust(16, '0') + f.read(__UDP_BUF_SIZE)
                __resend_until_ack(client, buffer, server)

                if bytes_sended == f_size:
                        break
                bytes_sended += len(buffer) - 32
